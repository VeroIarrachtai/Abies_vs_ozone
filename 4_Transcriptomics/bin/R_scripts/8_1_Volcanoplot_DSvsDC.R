...
ggplot(results_DESeq2_rt, aes(x=log2FoldChange, y=sig)) +
  ...
  geom_vline(xintercept = -1, colour = "black", linetype = "dashed", size = 0.25)  # Vertical significance cut-off line (+).
ggsave("../../outputs/8_1_VP_DE2_DSvsDC.png")
...
ggplot(results_Edge_rt, aes(x=logFC, y=sig)) +
  ...
  geom_vline(xintercept = 1, colour = "black", linetype = "dashed", size = 0.25)+  # Vertical significance cut-off line (+).
  geom_vline(xintercept = -1, colour = "black", linetype = "dashed", size = 0.25)  # Vertical significance cut-off line (+).
ggsave("../../outputs/8_1_VP_ER_DSvsDC.png")
...
# Export data 
write.table(overxpress_Ds2, "../../data/Over_Down/over_DE2_DSvsDC.txt", sep="\t", row.names=T)
write.table(downxpress_Ds2, "../../data/Over_Down/down_DE2_DSvsDC.txt", sep="\t", row.names=T)

write.table(overxpress_Ed , "../../data/Over_Down/over_ER_DSvsDC.txt", sep="\t", row.names=T)
write.table(downxpress_Ed , "../../data/Over_Down/down_ER_DSvsDC.txt", sep="\t", row.names=T)
