...
results_DESeq2<- read.delim("../../metadata/DGE/DESeq2_DSvsHS_FDR_5.txt") ## TENGO ESTO 2 VECES EN LA MISMA TABLA
results_Edge<- read.delim("../../metadata/DGE/EdgeR_DSvsHS_FDR_5.txt")
...
ggplot(results_Edge_rt, aes(x=logFC, y=sig)) +
  ...
  geom_vline(xintercept = 1, colour = "black", linetype = "dashed", size = 0.25)+  # Vertical significance cut-off line (+).
  geom_vline(xintercept = -1, colour = "black", linetype = "dashed", size = 0.25)  # Vertical significance cut-off line (+).
ggsave("../../outputs/8.1_VP_ER_S_t_vsD.png")
...
# Export data 
write.table(overxpress_Ds2, "../../metadata/DGE/oxs_DE2_S_TvsD.txt", sep="\t", row.names=T)
write.table(downxpress_Ds2, "../../metadata/DGE/sxs_DE2_S_TvsD.txt", sep="\t", row.names=T)

write.table(overxpress_Ed , "../../metadata/DGE/oxs_ER_S_TvsD.txt", sep="\t", row.names=T)
write.table(downxpress_Ed , "../../metadata/DGE/sxs_ER_S_TvsD.txt", sep="\t", row.names=T)
